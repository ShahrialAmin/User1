...
// Inline Mock Data (Fixing missing '../data' dependency)
const mockOrders = [
  {
    id: '1001',
    date: '28 Feb 2024',
...
export const TransactionsPage: React.FC = () => {
  ...
  const filteredTransactions = allTransactions.filter(trx => {
    const matchesTab = activeTab === 'All' || trx.status.toLowerCase() === activeTab.toLowerCase();
    
    const searchLower = searchQuery.toLowerCase();
    const matchesSearch = !searchQuery || 
        (trx.title && trx.title.toLowerCase().includes(searchLower)) ||
        (trx.id && trx.id.toLowerCase().includes(searchLower)) ||
        (trx.trxId && trx.trxId.toLowerCase().includes(searchLower)) ||
        (trx.method && trx.method.toLowerCase().includes(searchLower)) ||
        (trx.amount && trx.amount.toString().includes(searchLower));
  ...
  const handleReport = () => {
      ...
      const url = `https://wa.me/8801776927073?text=${encodeURIComponent(message)}`;
    ...
    <div className="max-w-5xl mx-auto md:px-6 lg:px-8 py-6">
      ...
      <div className="mb-4 relative px-3 md:px-0">
          ...
          <input
              ...
              placeholder="Search transaction ID, amount, or method..."
              value={searchQuery}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
             ...
             <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-xl border border-gray-100 border-dashed">
                ...
                <p className="text-slate-500 text-sm">
                    {searchQuery ? `No results for "${searchQuery}"` : `No transaction history available for ${activeTab}.`}
                </p>
            </div>
         )}
      </div>
